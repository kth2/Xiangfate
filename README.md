...
### 差异化：为什么曾经「换个人测，结果差不多」

多人试用后的反馈。把这句话拆成四层分别量过（`src/dev/differentiation.ts` +
`src/dev/__tests__/differentiation.test.ts`，跑 `DIFF_REPORT=/tmp/r.txt npm test` 出明细）：

| 层 | 曾经的问题 | 现在 |
|---|---|---|
| 测量 | 没问题 —— 两张脸的原始度量差得很开 | 不动 |
| 阈值 | 41 个五档区间里 15 个中和区半宽 > ±25%，吃掉九成以上的人 | 收窄到 3 个，且都不是真问题 |
| 规则 | 断语两两 Jaccard 相似度 49%，五维星级只有 19 种组合（1 星与 2 星从无人拿到） | Jaccard 23%，星级 120 种，五档都有人拿到 |
| Prompt | 骨架写死七段 + 每段字数配额，模型收到的输入只有 13.8% 因人而异 | 骨架按人算，200 人里 73 种提纲 |

修掉的确凿错误：
- `browGap`（印堂宽）中心写成 0.53，而实际测的是两眉头整段距离（实测 0.94–0.98）——
  判线低了 1.8 倍，于是**每个人**都判「印堂开阔」，两条相关断语永不命中。
- `openness`（虹膜可见度）判线照传统语感定在 0.62–0.85，而睑裂高普遍小于虹膜直径，
  该比值天然压在 0.59 —— 于是**每个人**都判「眼神含蓄」并领到忌相「目光藏（城府较深）」。
- `bridgeDeviation`（鼻梁弓度）判线离实测量级 11–52 倍，全部不可达，
  98% 的人领到贵格「梁柱端正」。已列入 `core/evidenceOnly.ts`：只报数，不判吉凶。

星级映射也重写过一轮。原式 `1 + (ratio − 0.1) / 0.8 × 4` 配 `balanced=0.75`，
「各项都在中和区间」恒落 4.25 星 —— 于是一律 4 星，1 星与 2 星从无人拿到。
现在零点固定在中和（3 星），各档只给相对偏移，中和项不摊薄真正的偏离，
证据薄的维度按 `E/(E+1)` 收缩。顺带纠正 `face.symmetry`：
它原来只有 high / balanced 两档，等于只能加分从不减分，在星级里一路往上推。

痣也从「一条压死」拆成了按位置分列：检测器一直能分出 17 个位置，
而规则层原来压成一条 `face.mole`（categorical），既没有权重、又因为
categorical 在星级里不参与，等于测到了也不算。现在每个位置各出一条特征、
各挂各的宫、各出一条断语；显痣（大而深）记 very_low，隐痣记 low ——
传统上隐痣不作重断。

五维权重表也清过一遍：**11 条特征测得出来、写进报告，却一个维度都没挂**
（法令纹、男女宫、婚姻线、土星丘、太阳丘、火星丘、眉骨、骨盆、太阳穴、臂展），
**骨相的「人际情感」合计权重为 0** —— 每份骨相报告这一维都恒定停在中性并标「未测到」。
面相的「执行意志」在鼻梁转为只报数后只剩一条特征在撑，最集中的星数占到 70%；
补齐后 10 条（7 条非像素），降到 50%。权重的依据一律是规则层自己写的 `meaning` 文本，
不另起一套量表。`core/__tests__/weights.test.ts` 直接从 rules 源码抽 id 做覆盖审计，
新增特征忘挂权重、权重键打错字、某维整维为空都会直接红。

仍待真实样本的：`thresholds.ts` 里所有标 `CALIBRATE` 的带宽（收窄过一轮，但仍是工程判断，
不是校准）；星级的证据先验 `EVIDENCE_PRIOR`；手相至今没有真实夹具。
仍然偏薄且**不打算硬凑**的：体相的「才智思辨」只有一条特征 —— 一张体照本来就说不出
一个人怎么思考，把体型接到才智上正是被现代研究否定的那套体质心理学，宁可薄。
...
实测：首屏 JS ≈ 100 KB gzip，应用壳预缓存 653 KiB，465 个测试全绿。
