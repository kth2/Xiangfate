 ...
 *   · boneFlesh.index   describe 明写「0.5 附近为骨肉相称」；上下两条线取 1/3 与 2/3，
 *                       理由是这条合成量只有两个来自自身定义的锚点 —— 两端（0 = 肉之极、
 *                       1 = 骨之极）与中点 0.5，而古法「骨胜／相称／肉胜」是三个并列的状态，
 *                       没有哪一态被说成更常见。既无分布先验，三等分就是不外加假设的切法。
 *                       换个说法：index − 0.5 恰是两个分量各自偏离自身中点的均值，
 *                       ±1/6 这条线即「两分量平均偏出自身量程的三分之一」。
 *                       ⚠️ 宽度仍是**暂定**的：三等分只是无先验下的默认，不是量出来的分位。
...
/**
 * ── 暂不收录（2026-08）─────────────────────────────────────
 * 下面这条写完之后被实测否掉了。留下记录，等真实样本到手再议 ——
 * **不要**为了让它能出象而挪判线，那正是本文件开头禁止的事。
 *
 * · **五岳朝拱 / 孤峰无辅**
...
export const FACE_CONFIGURATIONS: readonly ConfigurationSpec[] = [
  ...
  /**
   * 这一条曾被摘掉（原记录见头部「暂不收录」），现在放回来，理由是**两个分量都修好了**：
   *   · `cheekFullness` 原本拿侧廓（颧—颌角）连线去量正面颊部，量到的是脸的前后厚度 ——
   *     每张脸都约 54% IOD 而分母写 8%，于是恒为 1.0，指数退化成只由颌线转折决定。
   *     现改为颊部自身的矢高。
   *   · `jawAngularity` 原本取「颧→颌角→颌中」，比项目另外两处同名角高出约 9°，
   *     且映射 (160−x)/70 的跨度是另拍的。现改用同一组三点，并沿用骨相已有的
   *     T.gonialSquare / T.gonialSoft。
   * 修完之后 index 实测 0.331（真人脸）/ 0.574（规范脸），首次跨在 0.5 两侧。
   *
   * ⚠️ 判线本身仍未校准，三点务必记住：
   *   1. 1/3 与 2/3 取自「三态无先验则三等分」，不是分位数。宽度是暂定的。
   *   2. 合成人群没有三维特征，differentiation.test.ts 那条「没有哪一象命中九成以上」
   *      的守卫**覆盖不到这一条**。真实样本到手前，它的命中率分布是未知的。
   *   3. 真人脸 0.331 距 1/3 只差 0.002 —— 它落进「肉胜于骨」几乎是掷硬币。
   *      这个位置是判线定完之后才知道的，**不据此挪线**：按脸挪线正是本文件禁止的事。
   */
  {
    id: 'config.face.boneFlesh',
    name: '骨肉',
    domain: '根基',
    source: '太清神鉴',
    basis:
      '骨相三原则之一为「骨肉并重」：骨多肉少主劳碌孤峭，骨少肉多主意志偏缓，相称者刚柔并济。相书论骨从不单看骨，也不单看肉，只论二者之间的多寡',
    components: ['face3d.boneFlesh.index'],
    branches: [
      {
        configuration: '骨胜于肉',
        tone: 'ji',
        when: [
          {
            featureId: 'face3d.boneFlesh.index',
            gte: 0.667,
            requirement: '颌线转折硬而颊部不鼓，骨形显于肉外',
          },
        ],
        meaning: '骨多肉少，主一生劳碌、性孤而少亲，待人少回旋余地',
      },
      {
        configuration: '肉胜于骨',
        tone: 'ji',
        when: [
          {
            featureId: 'face3d.boneFlesh.index',
            lte: 0.333,
            requirement: '颊部外鼓而颌线转折柔和，肉厚而骨不显',
          },
        ],
        meaning: '骨少肉多，主意志偏缓、临事少断，安逸而少进取',
      },
      {
        configuration: '骨肉相称',
        tone: 'gui',
        when: [
          {
            ...
            gte: 0.333,
            lte: 0.667,
            requirement: '骨与肉相当，轮廓不峭不浮',
          },
        ],
        meaning: '骨肉相称，刚柔并济 —— 传统骨相以此为上',
        caution: '惟相称者亦无一处独强，遇须以偏取胜之事，反不如偏格者利',
      },
    ],
  },
] as const
