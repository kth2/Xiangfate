 ...
 *
 * · **骨肉相称 / 骨胜于肉 / 肉胜于骨**
 *   `boneFlesh.index` 实测 0.106 / 0.141，两张脸都落进「肉胜于骨」。
 *   而其 evidence 显示 `cheekFullness` 读到 **100.0%** —— 该分量已经饱和，
 *   合成指数因此被系统性压低。这是上游度量的问题，不是判线的问题：
 *   移动判线只会把一个饱和的量包装得像是测准了。
 *   已记入待查清单（real-face.test.ts 的饱和检查目前只盯 tipFullness 与 bridgeHeight，
 *   没有覆盖 cheekFullness）。
 * ────────────────────────────────────────────────────────
 */
