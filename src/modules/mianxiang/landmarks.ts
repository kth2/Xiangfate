...
/**
 * 颊部**正面**的一列取样点，自上而下：上缘 · 中心 · 下缘。
 *
 * ⚠️ 与 ZYGOMATIC / GONION 不是一回事 —— 那两个在 FACE_OVAL 上，属**侧廓**，
 * 在规范帧里位于面平面**后方**（实测颧 −39% IOD、颌角 −13% IOD）。
 * 拿侧廓两点连线去量正面颊部的偏移，量到的是「脸的前后厚度」，
 * 每张脸都有约 50% IOD，与胖瘦无关 —— 这正是 boneFlesh.cheekFullness
 * 曾经饱和的原因。要判颊部鼓还是凹，取样点必须都落在**同一片正面颊部**上。
 *
 * 校验方式与本文件其余索引一致：在规范帧里量各点的位置与前突量。
 * 三点沿颊部竖直排列（x 相近、y 递增），前突量呈中间高两端低的弧形 ——
 * 正是可以取矢高的形状。规范脸实测（左）：
 *     347  y=0.218  前突 28.8%
 *     425  y=0.471  前突 35.8%   ← 中
 *     432  y=0.805  前突 31.4%
 */
export const CHEEK_COLUMN = {
  left: { upper: 347, mid: 425, lower: 432 },
  right: { upper: 118, mid: 205, lower: 212 },
} as const
