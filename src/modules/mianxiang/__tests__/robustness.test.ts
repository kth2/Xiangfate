...
describe('小幅位姿扰动下的断语稳定性', () => {
  ...
  /**
   * 这两条不是断言「转多少度都不变」，而是记录**从哪个角度开始变**。
   * 失败时说明某条阈值贴得太近，应当调阈值或降置信度，而不是改这个测试。
   *
   * 门槛在 2026-08 的阈值收窄后往上挪了一截：yaw 原来 20° 就会翻，
   * 现在到 30° 都不动、35° 才变。同一次收窄也让「目光清明」这条贵格
   * 第一次变得可达（原判线在解剖上几乎到不了，见 T.openness）。
   * pitch 仍然敏感得多 —— 抬头低头直接改三停与鼻部的投影长度。
   */
  it('yaw 大到 35° 才会改变断语 —— 记录门槛，证明上面的稳定不是空转', () => {
    expect(verdictsOf(rotate(LM, { yaw: 30 }))).toEqual(BASE)
    expect(verdictsOf(rotate(LM, { yaw: 35 }))).not.toEqual(BASE)
  })

  it('pitch 15° 就会改变断语 —— 俯仰比偏转敏感得多', () => {
    expect(verdictsOf(rotate(LM, { pitch: 15 }))).not.toEqual(BASE)
  })
})
