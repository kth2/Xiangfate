...
const FACE_WEIGHTS: WeightMap = {
   ...
   * 痣按位置分列。
   *
   * 原来所有痣压成一条 `face.mole`（categorical），因此在星级里没有权重 ——
   * 给任何一个维度都是把「哪个位置」的信息丢掉之后瞎凑。现在规则层按位置
   * 逐条出特征（`face.mole.<slug>`），每个位置各归各的宫，权重也就有了依据。
   *
   * 每条的依据同样是规则层写的 MOLE_MEANING 原文，摘在注释里。
   * 档位一律是偏离档（low / very_low）—— 传统相术里痣基本都作忌处论，
   * 十七个位置的释义无一条是吉。
   */
  /** 印堂（命宫）见痣：「运途多阻、心事难解」 */
  'face.mole.yintang': { 才智思辨: 1, 气度格局: 1 },
  /** 额中见痣：「早年离祖、少得亲荫，凡事靠自己开局」 */
  'face.mole.ezhong': { 根基福泽: 2 },
  /** 额角（迁移宫）见痣：「奔波在外、居处多迁」 */
  'face.mole.ejiao': { 根基福泽: 1 },
  /** 眉中藏痣（草里藏珠）：「聪敏而有偏才，然兄弟之间易生嫌隙」 */
  'face.mole.mei': { 才智思辨: 1, 人际情感: 1 },
  /** 眼尾（奸门）见痣：「感情多波折、外缘纷扰」 */
  'face.mole.yanwei': { 人际情感: 2 },
  /** 眼下（男女宫）见痣：「为子女情事操心，泪堂有痣者心多牵挂」 */
  'face.mole.yanxia': { 人际情感: 2 },
  /** 山根见痣：「中年一道坎，事业须防中途生变」 */
  'face.mole.shangen': { 执行意志: 1, 根基福泽: 1 },
  /** 鼻梁（年寿）见痣：「中年运滞、财路易断，宜守不宜进」 */
  'face.mole.biliang': { 根基福泽: 2 },
  /** 准头见痣：「财帛易散、进多出亦多」 */
  'face.mole.zhuntou': { 根基福泽: 2 },
  /** 鼻翼（财库）见痣：「守财不易，钱财上须防漏」 */
  'face.mole.biyi': { 根基福泽: 2 },
  /** 颧上见痣：「权柄受挫、是非缠身，与人争位易招怨」 */
  'face.mole.quan': { 执行意志: 1, 人际情感: 1 },
  /** 人中见痣：「子息之事多操心，家中之事不由己」 */
  'face.mole.renzhong': { 根基福泽: 1, 人际情感: 1 },
  /** 唇周见痣：「主口福，亦主口舌是非，言多必失」 */
  'face.mole.chunzhou': { 人际情感: 2 },
  /** 法令见痣：「威令受阻、职事上多掣肘」 */
  'face.mole.faling': { 执行意志: 2 },
  /** 地阁见痣：「居所多迁、晚运奔波，田宅不易久守」 */
  'face.mole.dige': { 根基福泽: 2 },
  /** 腮上见痣：「性执而记怨，与下属部属之间易生龃龉」 */
  'face.mole.sai': { 人际情感: 2 },
  /** 颊上见痣：「人事纷扰，与人相处须留余地」 */
  'face.mole.mianjia': { 人际情感: 1 },
}
